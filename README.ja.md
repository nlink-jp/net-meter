...
グラフを描きます。棒は 1 秒に 1 本で、時間とともに左へ流れます。中央線より上が上り、下が下りです。
グラフの縦軸は通信量に追従します。大きな山が流れ去ったあとは、一度にではなく数秒かけて縮尺が戻ります。ダッシュは「この 1 秒には値が無い」こと、
...
インターフェースの申告値であって、実際に運べる量の上限ではありません。パネルを開いても、
作業中のアプリから net-meter に切り替わることはありません。開いている間のキー入力は
パネルが受け取ります。Esc で閉じるほか、パネルの外をクリックする、項目をもう一度クリックする、
のいずれでも閉じます。キーボード（⌘Tab）で別のアプリに切り替えたときは、次にクリックするまで
開いたままです。設定も同じパネルにあります。
